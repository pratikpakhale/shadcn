...
export default function RegistryPreview() {
  ...
  const removeFromLibrary = (libraryId: string, itemUrl: string) => {
    const updatedLibraries = libraries.map((lib) => {
      if (lib.id === libraryId) {
        return { ...lib, items: lib.items.filter((item) => item.url !== itemUrl) }
      }
      return lib
    })
    saveLibraries(updatedLibraries)
  ...
  const openAddToLibraryDialog = () => {
    ...
    const librariesWithUrl = new Set(
      libraries.filter((lib) => lib.items.some((item) => item.url === currentUrl)).map((lib) => lib.id),
    )
  ...
  const fetchRegistries = async () => {
    try {
      ...
      // Add custom registry
      const customRegistry = "https://21st.dev/"
  ...
  return (
    <div className="h-screen bg-background flex">
      ...
      <div className="w-80 border-r border-border flex flex-col">
        <div className="p-3 border-b border-border">
          ...
          <div className="relative">
            ...
            <Input
              placeholder={favoritesMode ? "Search libraries..." : "Search registries..."}
              value={searchQuery}
        ...
        <ScrollArea className="flex-1">
          {!favoritesMode ? (
            ...
            <div className="p-2">
              <div className="mb-2">
                <Button onClick={() => setShowCreateLibraryDialog(true)} size="sm" className="w-full" variant="outline">
              ...
              {libraries.length === 0 ? (
                <div className="p-8 text-center">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Create a library to organize your components</p>
                </div>
              ) : (
                libraries.map((library) => (
                  <div key={library.id} className="mb-2 border border-border rounded-lg overflow-hidden">
                          ...
                          library.items.map((item, idx) => (
                            ...
                            >
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-muted-foreground truncate">{item.registryName}</p>
                              </div>
                              <Button
                                size="sm"
                                variant="ghost"
          ...
          <>
            <div className="flex-1 bg-card relative group">
              ...
              <div className="absolute bottom-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                <Button onClick={openAddToLibraryDialog} size="sm" className="shadow-lg">
      ...
      <Dialog open={showAddToLibraryDialog} onOpenChange={setShowAddToLibraryDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {!extensionActive && (
                <span className="text-purple-500">
                  <a href="https://github.com/pratikpakhale/shadcn" target="_blank" className="underline">Install the extension</a> for correct URL tracking.
                </span>
              )}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            <div>
              <Label htmlFor="url-input" className="text-sm font-medium mb-2 block">
                URL {extensionActive && <Badge variant="secondary" className="text-xs ml-2">Auto-tracked</Badge>}
              </Label>
              <Input
                id="url-input"
                value={editableUrl}
            ...
            <div>
              <Label className="text-sm font-medium mb-2 block">Libraries</Label>
              {libraries.length === 0 ? (
                <div className="text-center py-8">
                  <p className="text-sm text-muted-foreground mb-4">No libraries yet. Create one first!</p>
                  <Button onClick={() => setShowCreateLibraryDialog(true)} variant="outline">
                    <FolderPlus className="h-4 w-4 mr-2" />
                    Create Library
                  </Button>
                </div>
                ...
                <ScrollArea className="max-h-[250px] border rounded-md p-3">
                  <div className="space-y-3">
                    {libraries.map((library) => {
                      const alreadyInLibrary = library.items.some((item) => item.url === editableUrl)
                      return (
                        <div key={library.id} className="flex items-center space-x-3">
                          ...
                          />
                          <Label htmlFor={library.id} className="flex-1 cursor-pointer">
          ...
          {libraries.length > 0 && (
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowAddToLibraryDialog(false)}>
                Cancel
              </Button>
              <Button onClick={addToLibraries} disabled={selectedLibraries.size === 0 || !editableUrl.trim()}>
                Add to {selectedLibraries.size} {selectedLibraries.size === 1 ? "Library" : "Libraries"}
              </Button>
            </DialogFooter>
          )}
        </DialogContent>
      ...
      <Dialog open={showCreateLibraryDialog} onOpenChange={setShowCreateLibraryDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Give your library a name to organize your favorite components.</DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowCreateLibraryDialog(false)}>
              Cancel
            </Button>
